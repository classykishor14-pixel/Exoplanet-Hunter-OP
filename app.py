...
<style>
  ...
  .stApp {
      ...
      background-size: 400px 400px, 100% 100%, 100% 100%, 100% 100%, cover;
      background-repeat: repeat, no-repeat, no-repeat, no-repeat, no-repeat;
      background-position: top left, center center, center center, center center, center center;
  ...
  .main .block-container {
      ...
      box-shadow: 0 8px 40px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.065), inset 0 -1px 0 rgba(0,0,0,0.30) !important;
  }
  ...
  div[data-testid="stExpander"], div[data-testid="stInfo"], div[data-testid="stSuccess"],
  ...
  div[data-testid="stPyplotRootElement"] {
      ...
      box-shadow: 0 4px 24px rgba(0,0,0,0.50), inset 0 1px 0 rgba(255,255,255,0.04) !important;
  }
  ...
  .stat-card {
      ...
      box-shadow: 0 4px 28px rgba(0,0,0,0.52), inset 0 1px 0 rgba(255,255,255,0.08), inset 0 -1px 0 rgba(0,0,0,0.22);
      transition: box-shadow 0.3s ease, border-color 0.3s ease, transform 0.2s ease;
  }
  .stat-card:hover {
      transform: translateY(-2px);
      box-shadow: 0 8px 36px rgba(0,0,0,0.62), 0 0 22px rgba(0,180,255,0.12), inset 0 1px 0 rgba(255,255,255,0.10);
  ...
  .stat-label {
      ...
      letter-spacing: 0.18em; text-transform: uppercase; color: #4a6890; margin-bottom: 5px;
  }
  .stat-value {
      font-family: 'Space Mono', monospace; font-size: 1.22rem; font-weight: 700; color: #eaf4ff;
      text-shadow: 0 0 14px rgba(0,180,255,0.22);
  }
  .stat-unit { font-size: 0.68rem; color: #4a6890; margin-left: 3px; }

  .stTextInput > div > div > input {
      background: rgba(6, 14, 34, 0.72) !important; backdrop-filter: blur(10px) !important;
      border: 1px solid rgba(40,70,120,0.58) !important; color: #c8d8f0 !important; border-radius: 8px !important;
  ...
  .stButton > button[kind="primary"] {
      background: rgba(0,212,255,0.08) !important; backdrop-filter: blur(10px) !important;
      border: 1px solid rgba(0,212,255,0.48) !important; color: #00d4ff !important;
      font-family: 'Space Mono', monospace !important; font-size: 0.78rem !important;
      letter-spacing: 0.1em !important; border-radius: 10px !important; transition: all 0.25s ease !important;
  }
  ...
  @keyframes glassIn { from { opacity: 0; transform: scale(0.97) translateY(12px); } to { opacity: 1; transform: scale(1.00) translateY(0); } }
  ...
  .delay-1 { animation-delay: 0.08s; } .delay-2 { animation-delay: 0.20s; } .delay-3 { animation-delay: 0.34s; }
...
# =============================================================================
# PIPELINE - UPDATED WITH TESS SUPPORT
# =============================================================================
...
def fetch_and_clean(target: str, mission: str, time_segment: int):
    ...
    if mission == "Kepler":
        ...
        err_msg = f"No Kepler data found for '{target}' Q{time_segment}."
    else:
        # TESS support added!
        result = lk.search_lightcurve(
            target, mission="TESS", sector=time_segment,
            author="SPOC", # SPOC is the standard pipeline for TESS
        )
        err_msg = f"No TESS data found for '{target}' Sector {time_segment}."
...
def plot_raw(raw_time, raw_flux, raw_ferr, trend_time, trend_flux):
    ...
    ax.plot(trend_time, trend_flux, color=C_TREND, lw=1.8, alpha=0.9, zorder=3, label=f"SG trend  (window={SG_WINDOW}, poly={SG_POLY})")
    ...
    ax.legend(loc="upper right", fontsize=8, framealpha=0.25, facecolor=BG_PANEL, edgecolor="#1e2d50", labelcolor="white")
...
def plot_flat(flat_time, flat_flux, clean_time, clean_flux):
    ...
    ax1.plot(flat_time, flat_flux, color=C_FLAT, lw=0.5, alpha=0.65, zorder=2, label="Flattened  (stellar trend removed)")
    ax1.axhline(1.0, color="#334", lw=0.7, ls="--", alpha=0.5)
    ax1.tick_params(labelbottom=False)
    ax1.set_ylabel("Flux", color=C_FLAT, fontsize=9)
    ax1.legend(loc="upper right", fontsize=8, framealpha=0.25, facecolor=BG_PANEL, edgecolor="#1e2d50", labelcolor="white")
    ax1.set_title("Flattened  (top)  →  After outlier removal  (bottom)", color=C_TICK, fontsize=9, loc="left", pad=6)

    ax2.plot(clean_time, clean_flux, color=C_CLEAN, lw=0.5, alpha=0.65, zorder=2, label="Planet-search ready  (outliers removed)")
    ...
    ax2.legend(loc="upper right", fontsize=8, framealpha=0.25, facecolor=BG_PANEL, edgecolor="#1e2d50", labelcolor="white")
...
def plot_bls(periods, power, clean_time, clean_flux, clean_ferr, best_period, best_t0, best_duration, best_depth):
    dur_h = best_duration * 24.0; half_dur_h = dur_h / 2.0; margin = max(best_depth * 6, 0.0015)
    ...
    top_ax.scatter([best_period], [peak_pwr], color=C_PEAK, s=80, zorder=5, label=f"Best period = {best_period:.5f} d")
    top_ax.annotate(f"P = {best_period:.5f} d", xy=(best_period, peak_pwr), xytext=(best_period + (BLS_MAX_PERIOD - best_period) * 0.10, peak_pwr * 0.86),
        color=C_ANNO, fontsize=8.5, fontweight="bold", arrowprops=dict(arrowstyle="->", color=C_ANNO, lw=1.0, connectionstyle="arc3,rad=0.2"),
        bbox=dict(boxstyle="round,pad=0.3", facecolor=BG_DARK, edgecolor=C_ANNO, alpha=0.85), zorder=6)
    ...
    top_ax.legend(loc="upper right", fontsize=8.5, framealpha=0.25, facecolor=BG_PANEL, edgecolor="#1e2d50", labelcolor="white")
    ...
    fold_ax.legend(loc="lower center", fontsize=7.5, framealpha=0.25, facecolor=BG_PANEL, edgecolor="#1e2d50", labelcolor="white")
    fold_ax.set_title("② Full Phase-Folded Curve", color=C_TICK, fontsize=9, pad=5, loc="left")

    zoom_h = dur_h * 4; zs = np.abs(phase_hours) <= zoom_h; zb = valid & (np.abs(bin_c) <= zoom_h); bottom_y = 1.0 - best_depth; arr_x = dur_h * 1.9
    ...
    zoom_ax.annotate("", xy=(-half_dur_h, bottom_y * 0.9999), xytext=(half_dur_h, bottom_y * 0.9999), arrowprops=dict(arrowstyle="<->", color="#88aaff", lw=1.0, mutation_scale=8))
    zoom_ax.text(0, bottom_y - margin * 0.35, f"Duration = {dur_h:.2f} h", color="#88aaff", fontsize=7.5, ha="center")
...
with st.sidebar:
    ...
    <div style='margin-bottom:1.6rem'>
      ...
      <div style='font-size:0.7rem;color:#2a4a6a;text-transform:uppercase;
        ...
        BLS Detection Engine
      </div>
    </div>
    """, unsafe_allow_html=True)

    # ── MISSION & TIMEFRAME SELECTOR (UPDATED) ────────────────────────────────
    st.markdown("---")
    st.markdown('<div class="sidebar-label">TELESCOPE MISSION</div>', unsafe_allow_html=True)
    
    # NEW TOGGLE FOR TESS vs KEPLER
    ...
    # DYNAMIC SLIDER based on Mission
    if selected_mission == "Kepler":
        time_segment = st.slider(
            "quarter_slider", min_value=0, max_value=17, value=6,
            label_visibility="collapsed", help="Kepler observed in 90-day quarters (Q0–Q17)."
        )
        st.markdown(
        ...
        time_segment = st.slider(
            "sector_slider", min_value=1, max_value=75, value=1,
            label_visibility="collapsed", help="TESS observes in 27-day sectors."
        )
        st.markdown(
            f"<div style='font-size:0.68rem;color:#2a4a6a;margin-top:-6px;margin-bottom:8px;'>"
            f"Selected Sector: <span style='color:#00d4ff'>S{time_segment}</span> &nbsp;·&nbsp; "
            f"Range: 1 – 75+</div>", unsafe_allow_html=True,
        )
    ...
    <div style='font-size:0.7rem;color:#1e3a5a;line-height:1.7;'>
      <b>Kepler Tests:</b> Kepler-22 · Kepler-16<br>
      <b>TESS Tests:</b> TOI-700 · WASP-126<br><br>
...
if not st.session_state.search_btn:
    ...
    st.session_state.star_name = st.text_input(
        ...
        placeholder="Enter Star Name (e.g., Kepler-10 or TOI-700)...",
...
<div class="animate-in" style='font-family:Space Mono,monospace;
  ...
  &nbsp;·&nbsp; {selected_mission.upper()} {"QUARTER" if selected_mission == "Kepler" else "SECTOR"} {time_segment}
</div>""", unsafe_allow_html=True)
...
with st.spinner("📡 Contacting NASA MAST archive and cleaning data …"):
    try:
        ...
        st.error(
            f"**Download failed:** {e}\n\n"
            "**Common fixes:**\n"
            "- Check star name spelling (e.g., `Kepler-22`, `TOI-700`)\n"
            "- Ensure the selected Mission matches the star name.\n"
            "- Try a different quarter/sector.\n"
        )
        st.session_state.search_btn = False
        st.stop()
...
Noise floor ≈ <b style='color:#a8ff78'>{noise_ppm:.0f} ppm</b> — BLS input.</div>""", unsafe_allow_html=True)
...
<div style='text-align:center;font-family:Space Mono,monospace;
  ...
  {st.session_state.star_name.upper()} · {"Q" if selected_mission == "Kepler" else "S"}{time_segment} · {len(clean_t):,} cadences ·
